...
export default function KakaoMiniMap() {
  const location = useSelector((state) => state.detail.info.detail1);
  let searchLocation;
  if (location) {
  ...
  // function geocoding() {
  //   geocoder.addressSearch(searchLocation, async function (result, status) {
  //     if (status === kakao.maps.services.Status.OK) {
  //       searchLocation = await new kakao.maps.LatLng(result[0].y, result[0].x);
